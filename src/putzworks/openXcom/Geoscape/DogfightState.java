...
public class DogfightState extends State
{
	...
	private Vector<int> _w1Dist, _w2Dist;
...
public DogfightState(Game game, Globe globe, Craft craft, Ufo ufo)
{
	...
	_currentDist = 640;
	_targetDist = 560;
	_w1Dist =  new Vector<int>();
	_w2Dist = new Vector<int>();
	_end = false;
	_destroy = false;
...
public void move()
{
	...
	for (int i = 0; i < _craft.getRules().getWeapons(); i++)
	{
		...
		for (Vector<int>.iterator d = wDist.begin(); d != wDist.end();)
		{
			// Update weapons
			(d) += 8;
	...
	for (int i = 0; i < _craft.getRules().getWeapons(); i++)
	{
		...
		for (Vector<int>.iterator d = wDist.begin(); d != wDist.end(); ++d)
		{
			for (int j = -2; j <= 0; j++)
			{
