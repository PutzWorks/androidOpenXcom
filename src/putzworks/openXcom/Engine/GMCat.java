...
static inline public void gmext_write_int16 (Vector<unsigned char> &midi,
	unsigned int n)
{
	midi.add(n >> 8);
	midi.add(n);
...
static inline public void gmext_write_delta (Vector<unsigned char> &midi,
	unsigned int delta)
{
	...
	while (--i)
		midi.add(data[i] | 0x80);

	midi.add(data[0]);
}

static inline public void gmext_write_tempo_ev (Vector<unsigned char> &midi,
	unsigned int tempo)
{
	midi.add(0xFF);
	midi.add(0x51);
...
static inline public void gmext_write_end_ev (Vector<unsigned char> &midi)
{
	midi.add(0xFF);
	midi.add(0x2F);
	midi.add(0);
...
static int gmext_write_sequence (Vector<unsigned char> &midi,
	...
	Vector<unsigned char> &midi)
{
	...
	for (int j=0; j<stream.ntracks; j++) {
		...
		midi.add(0);
		midi.add(0xB0 | stream.tracks[j].channel);
		for (int i=0; i<8; i++)
...
Music *GMCatFile.loadMIDI(unsigned int i)
{
	...
	unsigned char *raw = static_cast<unsigned char*> ((public void*)load(i));
